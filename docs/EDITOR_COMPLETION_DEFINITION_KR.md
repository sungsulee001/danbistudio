...
- local data directory, logs, crash diagnostics. 현재 Electron runtime은 userData 아래 `logs`, `crashDumps`, `projects`, `packages`, `renders`, `temp`를 만들고 `window.danbiEditor.system.diagnostics()` IPC와 smoke result JSON으로 노출한다. Settings Runtime Diagnostics panel also displays app/runtime/FFmpeg status and exposes userData, logs, crash dump, project, package, render, and temp paths with native copy/open/reveal actions.
...
- `npm run electron:install-smoke`로 NSIS installer를 workspace 내부 임시 설치 디렉터리에 silent install하고, 설치된 앱을 fresh userData로 실행해 sample project open, localhost-only renderer request, FFmpeg MP4 render, silent uninstall cleanup을 검증한다.
