...
import {
  inferCacheStorageMimeType,
  resolveReadableCachePath,
} from '@/server/cache-storage';
...
export async function GET(
  ...
  try {
    ...
    const filePath = resolveReadableCachePath(relativePath);
    const stats = await stat(filePath);

    if (!stats.isFile()) {
      return NextResponse.json({ error: 'Cache file was not found.' }, { status: 404 });
    }
    ...
    const headers = new Headers({
      ...
      'Content-Type': inferCacheStorageMimeType(filePath),
    });
