...
import {
  inferOutputStorageMimeType,
  resolveReadableOutputPath,
} from '@/server/output-storage';
...
export async function GET(
  ...
  try {
    ...
    const filePath = resolveReadableOutputPath(relativePath);
    const stats = await stat(filePath);

    if (!stats.isFile()) {
      return NextResponse.json({ error: 'Output file was not found.' }, { status: 404 });
    }

    const range = parseRangeHeader(request.headers.get('range'), stats.size);
    const contentType = inferOutputStorageMimeType(filePath);
